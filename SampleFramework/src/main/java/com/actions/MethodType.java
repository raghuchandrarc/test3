...
public class MethodType {
	...
	public void findMethod(String methodType, String objectLocators,
		...
		Class cl = null;
		try {
			cl = (Class) Class.forName("com.actions.MethodType");
			com.actions.MethodType clName = (MethodType) cl.newInstance();
	...
	private void findElementByCssSelector(String objectLocators) {
		...
		List<WebElement> list1 = Driver.getInstance().findElements(
	...
	public void findElementByXpath(String objectLocators) {
		...
		.until(ExpectedConditions.visibilityOfAllElements(Driver
	 ...
	 */
	public void click(MethodParameters model) throws InterruptedException {
		wait1(5000);
		WebDriverWait wait = new WebDriverWait(Driver.getDriver(), 30);
		wait.until(
	 ...
	 *Click on button/checkbox/radio button
	 */
	public void clickCheckbok(MethodParameters model) {
		wait1(5000);
		model.getElement().get(
				0).sendKeys(Keys.TAB);
	...
	public static String highLightElement(MethodParameters model) {
		JavascriptExecutor js = (JavascriptExecutor) Driver.getDriver();

		js.executeScript("arguments[0].setAttribute('style', 'background: yellow; border: 2px solid red;');", model.getElement().get(0));
		...
		js.executeScript("arguments[0].setAttribute('style','border: solid 2px white');", model.getElement().get(0));
		return "Pass";
	 ...
	 * @throws InterruptedException 
	 */
	public void enterText(MethodParameters model) throws InterruptedException {
Thread.sleep(3000);
		MainTestNG.LOGGER
	...
	public void verifyTitleOfPage(MethodParameters model) {
		MainTestNG.LOGGER.info("inside verifyTitleOfPage()" + "title=="
				+ Driver.getInstance().getTitle() + "data from excel="
				+ model.getData());

		wait1(2000);
		String actual = Driver.getInstance().getTitle().toString();
	 ...
	 * @throws Exception 
	 */
	public void readText(MethodParameters model) throws Exception {
		MainTestNG.LOGGER
	 ...
	 * @throws Exception 
	 */
	public void WriteTextToXl(MethodParameters model) throws Exception {
		MainTestNG.LOGGER
				.info("getText() method called  and value of getText==*************"
						+ model.getElement().get(0).getText());
		String GetTExt= model.getElement().get(0).getText();
		System.out.println("Get captured data "+GetTExt);
		ExcelLibrary.setcelldata(Practice.Path_TestData, Practice.Sheet_TestData,"Sample",GetTExt);
		MainTestNG.LOGGER.info("readText completed");
	}
	/**
	...
	public void switchToChildWindow(MethodParameters model) {
		...
		Set<String> windows = Driver.getInstance().getWindowHandles();
			...
			if (windows.size() > 1) {
				for (String child : windows) {
					if (!child.equals(parent)) {
						...
						if (Driver.getInstance().switchTo()
	...
	public void readUrlOfPage(MethodParameters model) {
		Driver.getInstance().getCurrentUrl();
	}
	...
	public void navigateToURL(MethodParameters model) {
		Driver.getInstance().navigate().to(model.getData());
	...
	public static WebElement waitForElement(By by) {
		...
		while (!(wait.until(ExpectedConditions.presenceOfElementLocated(by))
				.isDisplayed())) {
			wait = new WebDriverWait(Driver.getInstance(), 60);
	...
	public void dropDownByMouseHover(MethodParameters model) {
		...
		WebElement subElement = Driver.getInstance().findElement(
	...
	public void readTitleOfPage(MethodParameters model) {
		...
		titleOfPage = Driver.getInstance().getTitle();
	}
	...
	public void DragAndDrop(MethodParameters model) {
		...
		WebElement sourceElement = Driver.getDriver().findElement(
				By.xpath(actType[0]));
		WebElement destinationElement = Driver.getDriver().findElement(
				By.xpath(actType[1]));
	...
	public void webTableClick(MethodParameters model) {
		...
		WebElement mytable = Driver.getDriver().findElement(
				By.xpath(actType[0]));
	...
	public void selectDateFromCalendar(MethodParameters model) {
		...
		Driver.getDriver().findElement(By.xpath(actionType[0])).click();
		...
		while (dateNotFound) {
			...
			// and year then go Inside this condition.
				...
				Driver.getDriver().findElement(By.xpath(actionType[1]))
						.click();
			}
			...
			// month and year then go Inside this condition.
				...
				Driver.getDriver().findElement(By.xpath(actionType[2]))
						.click();
			}
		}
		wait1(3000);
	...
	public void selectDate(String date) {
		WebElement datePicker = Driver.getDriver().findElement(
